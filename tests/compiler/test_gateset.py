...
from hypothesis.strategies import sets, builds
...
def test_gate_set_get_general_sq_gate():
    ...
    gate_set = GateSet({ArbitraryCPhaseGate([3, 3]), VariableUnitaryGate(1, [3])})
...
def test_gate_set_intesection():
    ...
    assert gate_set.intersection({CNOTGate(), U3Gate()}) == {CNOTGate(), U3Gate()}
...
def test_gate_set_symmetric_difference():
    ...
    assert gate_set.symmetric_difference(gate_set2) == {CNOTGate(), U3Gate(), XGate(), YGate()}
...
def test_gate_set_or():
    ...
    assert gate_set | GateSet({CNOTGate(), U3Gate(), ToffoliGate()}) == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
    assert gate_set | GateSet({ToffoliGate()}) == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
...
def test_gate_set_or_other_set():
    ...
    assert gate_set | {CNOTGate(), U3Gate(), ToffoliGate()} == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
    assert gate_set | {ToffoliGate()} == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
...
def test_gate_set_xor():
    ...
    assert gate_set ^ GateSet({CNOTGate(), U3Gate(), ToffoliGate()}) == GateSet({ToffoliGate()})
    assert gate_set ^ GateSet({ToffoliGate()}) == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
    assert gate_set ^ GateSet({}) == gate_set
...
def test_gate_set_xor_other_set():
    ...
    assert gate_set ^ {CNOTGate(), U3Gate(), ToffoliGate()} == GateSet({ToffoliGate()})
    assert gate_set ^ {ToffoliGate()} == GateSet({CNOTGate(), U3Gate(), ToffoliGate()})
    assert gate_set ^ set() == gate_set

...
from bqskit.passes.control.predicates import OrPredicate
...
def test_true_and_true() -> None:
    ...
    pred = OrPredicate(TruePredicate(), TruePredicate())
...
def test_true_and_false() -> None:
    circuit = Circuit(1)
    data = PassData(circuit)
    pred = OrPredicate(TruePredicate(), FalsePredicate())
    assert pred.get_truth_value(circuit, data)
...
def test_false_and_true() -> None:
    circuit = Circuit(1)
    data = PassData(circuit)
    pred = OrPredicate(FalsePredicate(), TruePredicate())
    assert pred.get_truth_value(circuit, data)
...
def test_false_and_false() -> None:
    circuit = Circuit(1)
    data = PassData(circuit)
    pred = OrPredicate(FalsePredicate(), FalsePredicate())

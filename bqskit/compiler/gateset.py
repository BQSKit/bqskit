...
class GateSet(Set[Gate]):
    ...
    def __init__(self, gates: GateSetLike) -> None:
        ...
        if isinstance(gates, GateSet):
            self._gates = gates._gates.copy()
            self.radix_set = gates.radix_set.copy()
            return
    ...
    def single_qudit_gates(self) -> set[Gate]:
        ...
        return set(g for g in self if g.num_qudits == 1)

    @property
    def two_qudit_gates(self) -> set[Gate]:
        """All gates that act on two qudits."""
        return set(g for g in self if g.num_qudits == 2)
    ...
    def get_general_sq_gate(self) -> GeneralGate:
        ...
        During off-the-shelf BQSKit compilations, single-qudit gate
        rebasing is done last. The earlier stages of compilation focus
        on multi-qudit gates and use arbitrary single-qudit rotations.
        This function will return the best arbitrary single-qudit rotation
        to use during these steps based on the gates in the set.
        """
...
GateSetLike = Union[GateSet, set[Gate]]
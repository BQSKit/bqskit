...
class SaveIntermediatePass(BasePass):
    ...
    def run(self, circuit: Circuit, data: dict[str, Any]) -> None:
        ...
        structure_list: list[list[int, int]] = []
        ...
        for enum, block in blocks_to_save:
            enum = str(enum).zfill(num_digits)
            structure_list.append(block.location)
            subcircuit = Circuit(block.size)
            subcircuit.append_gate(block.gate, list(range(block.size)), block.params)
            ...
            if self.as_qasm:
                ...
                with open(block_skeleton + f'{enum}.pickle', 'wb') as f:
...
class RestoreIntermediatePass(BasePass):
    ...
    def reload_blocks(self):
        ...
        if len(self.block_list) > len(self.structure):
            raise ValueError('More block files than indicies in `structure.pickle`')
    ...
    def run(self, circuit: Circuit, data: dict[str, Any]) -> None:
        ...
        if circuit.get_depth() == 0:
            for block in self.block_list:
                # Get block
                block_num = int(findall('\d+', block)[0])
                ...
                if block_circ.get_size() != len(block_location):
                    raise ValueError(f"{block} and `structure.pickle` locations are different sizes.")
                # Append to circuit
                circuit.append_circuit(block_circ, block_location)
        # Check if the circuit has been partitioned, if so, try to replace blocks

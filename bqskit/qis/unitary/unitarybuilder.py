...
class UnitaryBuilder(Unitary):
    ...
    is an efficient way to string together or accumulate Unitary's.
    ...
    def __init__(self, num_qudits: int, radixes: Sequence[int] = []) -> None:
        ...
        if not is_valid_radixes(self.radixes, self.num_qudits):
    ...
    def get_unitary( self, params: Sequence[float] = [] ) -> UnitaryMatrix:
        """Build the unitary."""
        return self.tensor.reshape( ( self.get_dim(), self.get_dim() ) )

    def apply_right ( self, utry: UnitaryMatrix, location: Sequence[int], inverse: bool = False ) -> None:
        """
        ...
        utry = utry.get_dagger().numpy() if inverse else utry.numpy()
        ...
        shape = np.array(self.get_radixes() * 2)
        shape = shape.transpose(perm)
        self.tensor = self.tensor.reshape( shape )
        inv_perm = np.argsort( perm )
        self.tensor = self.tensor.transpose( inv_perm )

    def apply_left ( self, utry: UnitaryMatrix, location: Sequence[int], inverse: bool = False ) -> None:
        """
        ...
        mid_perm = [ x + self.get_size() for x in left_perm if x not in location ]
    ...
    def calc_env_matrix ( self, location: Sequence[int] ) -> np.ndarray:
        ...
        a = np.reshape( a, ( 2 ** ( self.get_size() - len( location ) ),

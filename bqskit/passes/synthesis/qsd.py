...
from bqskit.ir.gates.parameterized import RYGate, RZGate, VariableUnitaryGate
...
class FullQSDPass(PassAlias):
    ...
    any gates you want to decompose to VariableUnitaryGates before running this pass.

    Additionally, ScanningGateRemovalPass will operate on the context of the
    entire circuit. If your circuit is large, it is best to set `perform_scan` to False
    ...
    def __init__(
            ...
            self.scan: TreeScanningGateRemovalPass | ScanningGateRemovalPass = ScanningGateRemovalPass(start_from_left=start_from_left, 
    ...
    async def run(self, circuit: Circuit, data: PassData) -> None:
        """ Run a round of QSD, Multiplex Gate Decomposition, 
        and Scanning Gate Removal (optionally) until you reach the desired qudit size gates."""
...
class MGDPass(BasePass):
    ...
    def decompose(op: Operation) -> Circuit:
        '''
        Return the decomposed circuit from one operation of a 
        multiplexed gate.
        ...
        gate: MCRYGate | MCRZGate | RYGate | RZGate = MCRZGate(op.num_qudits - 1, 
                                                               op.num_qudits - 2)
        if (op.num_qudits > 2):
...
class QSDPass(BasePass):
    ...
    This decomposition takes each unitary of size n and decomposes it into a circuit
    with 4 VariableUnitaryGates of size n - 1 and 3 multiplexed rotation gates.
    ...
    @staticmethod
    def create_unitary_gate(u: UnitaryMatrix) -> tuple[VariableUnitaryGate, RealVector]:
        """
    ...
    def create_multiplexed_circ(us: list[UnitaryMatrix], select_qubits: list[int], controlled_qubit: int) -> Circuit:
        '''
        Takes a list of 2 unitaries of size n. Returns a circuit that
        decomposes the unitaries into a circuit with 2 unitaries of size n-1 and a 
    ...
    def mod_unitaries(u: UnitaryMatrix) -> UnitaryMatrix:
        '''
        Apply a permutation transform to the unitaries to the rest of the circuit.
        '''
    ...
    def qsd(orig_u: UnitaryMatrix) -> Circuit:
        ...
        # Perform CS Decomposition to solve for multiplexed unitaries and theta_y
        ...
        # This generates 2 circuits that multipex U and V with an MCRY gate in between

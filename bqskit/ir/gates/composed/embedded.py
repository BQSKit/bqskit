...
from bqskit.utils.typing import is_integer, is_valid_radixes


class EmbeddedGate(ComposedGate, QuditGate, DifferentiableUnitary):
    """
    An arbitrary qudit gate composed from embedding a lower qudit gate into a higher qudit, eg qubit gate into qutrit gate.
    ...
    def __init__(
        ...
        if not is_integer(target_radixes) and type(target_radixes) != Sequence[int]:
            raise TypeError('Expected target radixes to be integer or a sequence of integers, got %s.' % type(target_radixes))
    
        if type(level_map) != Sequence[int] and type(level_map) != Sequence[Sequence[int]]:
            raise TypeError('Expected level map to a sequence of integers or a sequence of sequences of integers, got %s.' % type(level_map))
        ...
        if not is_valid_radixes(target_radixes, gate.radixes):
            raise ValueError('Target radixes must have the same length as gate.radixes.')
    
        for i in range(len(target_radixes)):
            if target_radixes[i]<gate.radixes[i]:
                raise ValueError('Target radix at index %s should be greater than or equal to gate radix at same index.'%(i))
        
        if len(gate.radixes)==1:
            if len(level_map)!=1 and len(level_map)!=gate.radixes[0]:
                raise ValueError(
                    'Level map must be a sequnce of %s integers, or a sequence with one sequence element of %s integers.' % (gate.radixes[0],gate.radixes[0]))
        elif len(level_map)!= len(gate.radixes):
            raise ValueError('Level map must have the same length as gate.radixes.')
    
        for i in range(len(level_map)):
            if len(level_map[i] != gate.radixes[i]):
                raise ValueError('Level map at index %s must have %s integers to match respective radix of gate' %(gate.radixes[i],gate.radixes[i]))
            if np.any(level_map[i]>=gate.radixes[i]):
                raise ValueError('One or more elements of level map at index %s is greater than gate radix at the same index with value %s.'(i, gate.radixes[i])) 
    ...
    def _map_gate_to_target_radixes(self,Matrix,Unitary):
        ...
        gate.radixes[i] -> radixes[i]
        the j-th level of radixes[i] -> level_map[i][j] 
        """
        if len(self.gate.radixes)==1:
            ...
            raise NotImplementedError("Currently we support up to 3 qudit gate embedding")
        return Matrix
    ...
    def get_grad(self, params: RealVector = []) -> npt.NDArray[np.complex128]:
        ...
        M=np.zeros((np.prod(self.radixes),np.prod(self.radixes)),dtype=np.complex128)
        result = []

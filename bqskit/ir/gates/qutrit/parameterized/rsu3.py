...
class RSU3Gate(QutritGate, DifferentiableUnitary, CachedClass):
    ...
    def __init__(self, index: int):
        ...
        self.radixes = tuple([self.num_levels] * self.num_qudits)
        self.index = index
        
    def get_unitary(self, param: RealVector = []) -> UnitaryMatrix:
    ...
    def get_grad(self, params: RealVector = []) -> npt.NDArray[np.complex128]:
        ...
        if self.index == 0:
            ...
            matrix[0,0] = -1j/np.sqrt(3)*np.exp(-1j*params[0]/np.sqrt(3))
            matrix[1,1] = -1j/np.sqrt(3)*np.exp(-1j*params[0]/np.sqrt(3))

...
class PairGate(QutritGate, DifferentiableUnitary, CachedClass):
    """The G rotation double qutrit gate with the same generator on each qutrit (Gate = exp(-i sum_j param_j dot G_j x G_j) )."""
    ...
    def _unitary(self, params: RealVector = []):
        first_one_body=jax.numpy.sum(jax.numpy.array([params[i]*jax.numpy.kron(Lambda[i+1],jax.numpy.eye(3)) for i in range(8)]),axis=0)
        second_one_body=jax.numpy.sum(jax.numpy.array([params[i+8]*jax.numpy.kron(np.eye(3),Lambda[i+1]) for i in range(8)]),axis=0)
        two_body = params[16]*jax.numpy.sum(jax.numpy.array([jax.numpy.kron(Lambda[i+1],Lambda[i+1]) for i in range(8)]),axis=0)
        tot = first_one_body + second_one_body + two_body
        return jax.scipy.linalg.expm(-1j*tot)

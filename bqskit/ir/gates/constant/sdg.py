...
class SdgGate(QuditGate):
    """
        The single-qudit S Dagger gate 

        This gate is equivalent to a Pauli S dagger gate on the subspace of levels i,j.
    ...
    def __init__(self, num_levels: int=2, level_1: int=0, level_2: int=1):
        if num_levels <2 or not is_integer(num_levels):
            raise ValueError(
                'SdgGate num_levels must be a postive integer greater than or equal to 2.',
            )
        self.num_levels = num_levels
        if level_1 > num_levels or level_2 > num_levels:
            raise ValueError(
                'SdgGate indices must be equal or less to the number of levels.',
            )

...
class DetachedServer:
    ...
    In detached mode, the runtime is started separately than the client.
    Clients can connect and disconnect but not shutdown a detached server.
    This architecture is designed for the distributed setting, where
    managers run in shared memory on nodes and communicate with the server
    over the network.
    """
    ...
    def __init__(self, ipports: list[tuple[str, int]]) -> None:
        ...
        self.step_size = (self.upper_id_bound - self.lower_id_bound) // len(ipports)
        for i, (ip, port) in enumerate(ipports):
            lb = self.lower_id_bound + (i*self.step_size)
            ub = min(self.lower_id_bound + ((i+1)*self.step_size), self.upper_id_bound)
    ...
    def _recieve_new_comp_task(self, conn: Connection, task: CompilationTask) -> None:
    ...
    def _recieve_new_tasks(self, tasks: list[RuntimeTask]) -> None:
        ...
        assignments = [[] for _ in self.managers]
        for task in tasks:
            # select manager
            min_tasks = max(self.manager_idle_resources)
            best_id = self.manager_idle_resources.index(min_tasks)
...
def start_detached_server(*args, **kwargs) -> None:
